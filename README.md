...
### 2.2 開発の変遷
- **Gen.0（画像ランダム版）**：動作確認用の簡易プロトタイプ。ランダムな画像表示のみ。
- **Gen.1（二進数コア版）**：二進数表示と紙カード連動の基本構造を確立。抽選ロジックと待機演出を実装。
- **Gen.2（UIオマージュ版）**：「恋みくじ v3」の和風UIを参考に、朱色の札・縦書き・ホバーアニメーションをCSSで再現。
- **Gen.3（運用最適化版）**：札の枚数を16枚に拡張し、レスポンシブ対応を強化。固定ヘッダーとハンバーガーメニューを追加。
- **Gen.4（機能拡張版）**：**リセットボタン（「次の方へ」）を追加**し、共用端末での連続利用をスムーズにした。抽選中であっても強制的に初期状態に戻せる設計。
- **Gen.5 (大幅アップデート版)**:コンセプトとの整合性を考慮し**二進数要素を撤廃し「e-mikuji」に改称**、さらに**結果表示をポップアップ式に変更**。
